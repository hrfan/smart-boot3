...
public class SmartUserServiceImpl extends ServiceImpl<SmartUserMapper, SmartUser> implements SmartUserService {
     ...
     */
    @Override
    public SmartUser getUserByName(String username) {
        LambdaQueryWrapper<SmartUser> eq = new LambdaQueryWrapper<SmartUser>()
                .eq(SmartUser::getUsername, username);
        SmartUser smartUser = baseMapper.selectOne(eq);
        if (smartUser == null) {
