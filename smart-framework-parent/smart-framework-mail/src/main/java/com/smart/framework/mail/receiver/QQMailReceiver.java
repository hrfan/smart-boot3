...
import com.smart.framework.mail.base.BaseMailReceiver;
 ...
 * QQ邮件接收实现类
 */
public class QQMailReceiver extends BaseMailReceiver {
    private static final Logger logger = Logger.getLogger(QQMailReceiver.class.getName());

    public QQMailReceiver(MailConfig config) {
        super(config);
    }

    @Override
    protected List<MailInfo> fetchLatestEmails(int count) {
        logger.info("通过QQ获取最新邮件，数量：" + count);
        // QQ邮件接收使用父类的通用实现
    ...
    protected MailInfo fetchMailDetail(String mailId) {
        logger.info("通过QQ获取邮件详情，邮件ID：" + mailId);
        // QQ邮件详情获取使用父类的通用实现
    ...
    protected boolean performDownloadAttachment(String mailId, int attachmentIndex, String savePath) {
        logger.info("通过QQ下载附件，邮件ID：" + mailId + "，附件索引：" + attachmentIndex);
        // QQ附件下载使用父类的通用实现
    ...
    protected boolean performMarkAsRead(String mailId) {
        logger.info("通过QQ标记邮件为已读，邮件ID：" + mailId);
        // QQ标记已读使用父类的通用实现
        // 父类已经实现了完整的标记已读逻辑
    ...
    protected boolean performDeleteMail(String mailId) {
        logger.info("通过QQ删除邮件，邮件ID：" + mailId);
        // QQ删除邮件使用父类的通用实现
        // 父类已经实现了完整的删除邮件逻辑
